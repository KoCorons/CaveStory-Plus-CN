...
local function ActBossCharT_DragonBody(npc)
	...
	elseif npc.act_no == 100 then
		...
		npc.x = npc.x + cs.div((npc.tgt_x - npc.x), 8)
		npc.y = npc.y + cs.div((npc.tgt_y - npc.y), 8)
		if npc.x > cs.gMC[1 + npc.tgt_mc].x then
			npc.direct = cs.DIR_LEFT
		else
			npc.direct = cs.DIR_RIGHT
...
local function ActBossCharT_DragonHead(npc)
	...
	elseif npc.act_no == 220 then
		...
		if cs.mod(npc.act_wait, 8) == 1 then
			...
			xm = cs.GetCos(deg)
			if npc.direct == cs.DIR_LEFT then
				cs.SetNpChar(202, npc.x - 8 * cs.VS, npc.y, xm, ym, 0, nil, cs.div(cs.MAX_NPC, 2))
			else
				cs.SetNpChar(202, npc.x + 8 * cs.VS, npc.y, xm, ym, 0, nil, cs.div(cs.MAX_NPC, 2))
			end
			cs.PlaySoundObject(33, 1)
		end
...
-- Main Twin boss
function ActBossChar_Twin()
	local npc = cs.gBoss[1]
	...
	if npc.act_no == 0 then
		...
		cs.gBoss[3].cond = cs.gBoss[3].cond | 0x10
		cs.gBoss[3].damage = 10

		cs.gBoss[4].cond = 0x80
	...
	elseif npc.act_no == 401 then
		...
		elseif npc.act_wait < 500 then
			npc.count1 = npc.count1 + 4
		elseif npc.act_wait < 540 then
			npc.count1 = npc.count1 + 2
		elseif npc.act_wait < 560 then
			npc.count1 = npc.count1 + 1
		else
			npc.act_no = 100
			npc.act_wait = 0
	...
	if npc.act_no == 1001 then
		...
		cs.SetNpChar(4, npc.x + cs.Random(-128, 128) * cs.VS, npc.y + cs.Random(-70, 70) * cs.VS, 0, 0, 0, nil, cs.div(cs.MAX_NPC, 2))
